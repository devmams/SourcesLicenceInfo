	...
	public static void main(String[] args) {
		...
		while (!victoire() && tour<Constantes.TourMax) {
			...
			for (int i=0; i<tour; i++) {
				int[] p = new int[6];
				p[0] = 12; //Math.abs(rand.nextInt())%Constantes.Largeur; // abscisse
				p[1] = 0+i;//Math.abs(rand.nextInt())%Constantes.Hauteur; // ordonnée
				p[2] = 50;//Math.abs(rand.nextInt())%(Constantes.PlaneteTailleMax-Constantes.PlaneteTailleMin+1) + Constantes.PlaneteTailleMin; // taille
				p[3] = 210;//Math.abs(rand.nextInt())%256; // composante rouge
				p[4] = 0;//Math.abs(rand.nextInt())%256; // composante vert
				p[5] = 0;//Math.abs(rand.nextInt())%256; // composante bleu
				listePlanetes.add(p);
			}
			for (int i=0; i<tour; i++) {
				int[] p = new int[6];
				p[0] = 10;//Math.abs(rand.nextInt())%Constantes.Largeur; // abscisse
				p[1] = 0+i;//Math.abs(rand.nextInt())%Constantes.Hauteur; // ordonnée
				p[2] = 10;//Math.abs(rand.nextInt())%(Constantes.VaisseauResistanceMax-Constantes.VaisseauResistanceMin+1) + Constantes.VaisseauResistanceMin; // résistance
				p[3] = 210;//Math.abs(rand.nextInt())%256; // composante rouge
				p[4] = 0;//Math.abs(rand.nextInt())%256; // composante vert
				p[5] = 0;//Math.abs(rand.nextInt())%256; // composante bleu
