...
-- 684J_DIALLO_MAMADOU_PSA_2018.sql
...
select velo_id , date_achat , count(*) "nb"
from Velos natural join LocationsPassees
group by velo_id , date_achat;
...
CREATE OR REPLACE PROCEDURE CalculDuree(date_location date,date_rendu date)
IS
BEGIN
	dbms_output.put_line(TO_DATE(date_location,'DD-MM-YYYY HH24:MI') - TO_DATE(date_rendu,'DD-MM-YYYY HH24:MI'));
END;
/
show err
execute CalculDuree(60*TO_DATE('29-03-2018 15:00','DD-MM-YYYY HH24:MI'),TO_DATE('20-03-2018 16:30','DD-MM-YYYY HH24:MI'));
...
CREATE OR REPLACE TRIGGER trigger_rendu
  AFTER INSERT OR UPDATE ON LocationsPassees
  FOR EACH ROW
  DECLARE
  	d NUMBER;
  BEGIN
  SELECT date_loc INTO LocationsPassees
  WHERE id_veh = :new.id_veh;
  SELECT date_ren INTO LocationsPassees
  WHERE id_veh = :new.id_veh;
  IF
  END;
/
