...
public class Balle {
	...
	public Point CalculePointAtterissageBalle(Point pdepart,Point pcible){
		double x,y;
		Vecteur vInit = new Vecteur(pdepart,pcible);
		Vecteur vAbs = new Vecteur(new Point(0,0),new Point(10,0));
		
		
		
		double angleInit = vAbs.angle(vInit);
		double angleNouv = Math.toRadians(angleModifie(Math.toDegrees(angleInit)));
		double distanceInit = vInit.norme();
		...
		double distanceRatio = distanceNouv/distanceInit;
		Vecteur vFinal = vInit.appliquerAngle(angleNouv-angleInit).mult(distanceRatio);
		x = vFinal.v1+pdepart.x;
		y = vFinal.v2+pdepart.y;
	...
	public double angleModifie(double angle){
		...
		if(alea == 40){
			System.out.println("angle raté de beaucoup");
		}
		else{
			System.out.println("angle raté de peu");
		}
		if(res == 0){
			return angle-alea; 
		}
