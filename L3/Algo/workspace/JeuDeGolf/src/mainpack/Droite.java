/**
 * Classe permettant de créer une droite
 * 
 * @author Nicolas VANNIER / Mamadou DIALLO
 */
package mainpack;

public class Droite {
	
	protected double a,b,c;
	
	/**
	 * Construit une droite à partir de deux points
	 * @param deux points appartenant à la droite
	 */
	public Droite(Point A,Point B) {
		CalculCoefficientsDroite(A,B);
	}
	
	/**
	 * Construit une droite à partir de deux points
	 * @param deux points appartenant à la droite
	 */
	public Droite(double a,double b,double c){
		this.a = a;
		this.b = b;
		this.c = c;
	}
	
	/**
	 * determine l'angle entre deux droites
	 * @param l'autre droite
	 * @return un réel
	 */
	public double angleEntreDeuxDroite(Droite autre){
		double angleDegre;
		angleDegre = (((a*autre.b)-(autre.a*b))/((a*autre.a)-(b*autre.b)));
		return 	Math.atan(angleDegre);
	}
	
	/**
	 * Calcule les coefficients a,b,c d'une droite
	 * @param deux point appartenant à la droite
	 */
	public void CalculCoefficientsDroite(Point A, Point B){
		b = -B.x + A.x;
		a = B.y - A.y;
		c = -a*A.x-b*A.y;
	}
	
	/**
	 * Affiche l'équation cartésienne d'une droite
	 */
	public void afficheDroite(){
		System.out.println(a+ "X + "+ b +"Y +"+c+" = 0" );
	}
	
	
	/**
	 * Vérifie l'appartenance d'un point à une droite
	 * @param le point p
	 * @return un entier
	 */
	public int appartient(Point p){
		double res = a*p.x+b*p.y+c;

		// Tolérance pour l'appartenance d'un point à la droite.
		if(Simulation.round(Math.abs(res),5)==0) {
			return 0;
		}else if (res>0) {
			return 1;
		}else {
			return -1;
		}
	}
	
	/**
	 * Vérifie si deux droites s'intersectent
	 * @param l'autre droite d
	 * @return un booleen "true" si y a intersection et "false" sinon
	 */
	public boolean intersecte(Droite d){
		if(a==0 && d.a==0){
			return false;
		}else if(b==0 && d.b==0){
			return false;
		}else if(b==0 || d.b==0){
			return true;
		}else if(a/b==d.a/d.b){
			return false;
		}else{
			return true;
		}
	}
	
	/**
	 * Calcule le point d'intersection entre deux doites
	 * @param l'autre droite d
	 * @return le point d'intersection
	 */
	public Point pointIntersection(Droite d){
		double x,y;
		if(this.intersecte(d)){
			x = 0;
			y = 0;
			if(a == 0 && (b != 0 )){
				y = -(c/b);
				if(d.a != 0)
					x = (((-d.b/d.a)*y)-(d.c/d.a));
			}
			else if(b == 0 && a != 0){
				x = -(c/a);
				if(d.b != 0)
					y = (((-d.a/d.b)*x)-(d.c/d.b));
			}				
			else if(d.a == 0 && d.b != 0){
				y = -(d.c/d.b);
				if(a != 0)
					x = (((-b/a)*y)-(c/a));
			}
			else if(d.b == 0 && d.a != 0){
				x = -(d.c/d.a);
				if(b != 0)
					y = (((-a/b)*x)-(c/b));
			}
			else{
				x = ((d.c/d.b)-(c/b))/((a/b)-(d.a/d.b));
				y = ((-a/b)*x) - (c/b);
			}
			return new Point(x, y);
		}
		else{
			return new Point();
		}
	}
}